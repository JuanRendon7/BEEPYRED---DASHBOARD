...
export function ZonesDrawer({ isOpen, onClose }: ZonesDrawerProps) {
  ...
  const title = data ? `Clientes por zona (${data.totalZonas})` : 'Clientes por zona'
  ...
  return (
    <>
      ...
      >
        ...
        {/* Zona con más clientes — destacada */}
        {!isLoading && !isError && data && data.zonaConMasClientes && (
          <div className="flex-shrink-0 px-5 py-3 border-b border-border bg-brand/5">
            ...
            <p className="mt-1 text-sm font-semibold text-text-primary">{data.zonaConMasClientes}</p>
            {zones[0] && (
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {isError && (
            <div className="p-5">
              <p className="text-sm text-error">Error al cargar zonas</p>
          ...
          {!isLoading && !isError && zones.length > 0 && (
            <table className="w-full text-sm">
              <thead className="sticky top-0 bg-surface border-b border-border">
                <tr>
                  <th className="px-5 py-3 text-left text-xs font-medium text-text-muted uppercase tracking-wider">Zona</th>
